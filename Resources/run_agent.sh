...
then
	...
	then
		...
		g++ -std=c++11 $submit_folder/src/*.cpp -o $submit_folder/bin/Minesweeper &> /dev/null
	...
	then
		...
		then
			...
			cp -a $submit_folder/src/mine.jar. $submit_folder/bin
...
then
	if [ -f $submit_folder/bin/Minesweeper ]
	then
		$timeout_script -t $timeout_value $submit_folder/bin/Minesweeper -f $world_folder $submit_folder/Results.txt &> /dev/null
	...
	elif [ -f $submit_folder/bin/Minesweeper.jar ]
	then
		$timeout_script -t $timeout_value java -jar $submit_folder/bin/mine.jar -f $world_folder $submit_folder/Results.txt &> /dev/null
