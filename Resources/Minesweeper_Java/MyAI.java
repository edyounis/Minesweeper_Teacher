...
public class MyAI extends AI {
	...
	private boolean contradictionSearch(Coordinate tile) {
		...
		// Assume (x,y) is a bomb (and flag it). Try to derive contradiction
	...
	private boolean contradiction(HashMap<String, Integer>[][] boardInfo, BOMB_INFO[][] bombInfo) {
		 ...
		 * 
		 * Note: This implementation only detects type 1.
		 */
	...
	private HashMap<String, Integer>[][] deepCopy(HashMap<String, Integer>[][] boardInfo) {
		/*
		 * Creates a deep copy of this.boardInfo
		 */
		@SuppressWarnings("unchecked")
	...
	private LinkedList<Coordinate> findBombsFromCounts(HashMap<String, Integer>[][] boardInfo, BOMB_INFO[][] bombInfo) {
		/* Finds (new) bombs by comparing how many covered neighbors there are for each tile.
		 * If for some (uncovered) tile, tile.number is equal to the number of covered neighbors, then 
		 * all such covered neighbors are bombs.
		 * Pseudocode:
		 * 
		 * newBombs = []
		 * if tile[i][j].number == tile[i][j].coveredTiles:
		 * 		if bombInfo[i][j] == UNKNOWN
		 * 			newBombs.append(tile[i][j])
		 * 
		 * return newBombs
		 */
		...
		for (int y = 0; y < boardInfo.length; y++) {
			for (int x = 0; x < boardInfo[0].length; x++) {
				...
				if (tile.get("number") == tile.get("covered_neighbors")) {
					...
					// Add the covered neighbors that have bomb status UKNOWN
					for (Coordinate nb : allNeighbors) {
	...
	private void flagBombsFromCounts(HashMap<String, Integer>[][] boardInfo, BOMB_INFO[][] bombInfo) {
		/*
		 * For each new bomb found (based on covered neighbor counts), add a UNFLAG action to the
		 * moveQ.
		 */
	...
	private LinkedList<Coordinate> findSafeTilesFromCounts(HashMap<String, Integer>[][] boardInfo, BOMB_INFO[][] bombInfo) {
		/*
		 * Finds safe tiles to uncover.
		 * 
		 * For each tile, compare tile.number to the # of neighbors known to be bombs.
		 * If they are equal, then all unflagged + covered neighbors are safe.
		 */
	...
	private void uncoverSafeTiles(HashMap<String, Integer>[][] boardInfo, BOMB_INFO[][] bombInfo) {
		/*
		 * For each safe tile found (based on neighboring bombs counts), add an UNCOVER action to the
		 * moveQ.
		 */
		LinkedList<Coordinate> safeTiles = this.findSafeTilesFromCounts(boardInfo, bombInfo);
	...
	private void clearZeroPatch(int x, int y) {
		/*
		 * For a tile with number "0", this function add UNCOVER actions for each of the tile's
		 * (covered) neighbors.
		 */
		LinkedList<Coordinate> neighbors = getAllNeighbors(x,y);
		for (Coordinate nbr: neighbors) {
			if (this.boardInfo[nbr.y][nbr.x].get("number") == - 1) {
				Action a = new Action(ACTION.UNCOVER, nbr.x, nbr.y);
	...
	private boolean inBuffer(Action a) {
		/*
		 * Checks if an action is already in the moveQ
		 */
	...
	private void initBoardInfo() {
		/*
		 * Call this function only after the dimensions have been set
		 */
	...
	// ######### For Debugging Contradiction Searching ##########
